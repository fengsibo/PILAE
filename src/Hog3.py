...
def extract_hog_featuer_3(X, type, num):
    input_X = X
    from skimage.feature import hog
    hog_f = hog(input_X[0][0], hog_descriptor[num]["orientations"], hog_descriptor[num]["block"], hog_descriptor[num]["cell"])
    hog_f = hog_f.reshape((hog_f.shape[0], 1))
    feature = np.empty(hog_f.shape)
    i = 0
    for arr in input_X:
        for j in range(3):
            hog_f = hog(arr[j], hog_descriptor[num]["orientations"], hog_descriptor[num]["block"], hog_descriptor[num]["cell"])
            hog_f = hog_f.reshape((hog_f.shape[0], 1))
            print(i)
            i += 1
            feature = np.append(feature, hog_f, axis=1)
...
if __name__ == "__main__":
    ...
    (X_train, y_train), (X_test, y_test) = tools.load_npz("../dataset/cifar-10/cifar10_gray.npz")

    # hog_f = hog(X_train[0][0], 24, (12, 12), (2, 2))
    # print(hog_f)
    ...
    i = 0
    while i < len:
        extract_hog_featuer(X_train, "cifar-10/cifar10_hog_feature_train", i)
        extract_hog_featuer(X_test, "cifar-10/cifar10_hog_feature_test", i)
        i += 1
